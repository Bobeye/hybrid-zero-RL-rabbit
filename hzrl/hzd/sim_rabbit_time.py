...
class Policy():
    def __init__(self, theta=None, action_size=4, action_min=-4., action_max=4., kp=120, kd=2, feq=20.):
        ...
        self.p = params_time.p
        self.time_range = params_time.p[0] - params_time.p[1]
    ...
    ######MODIFICATIONS HERE
        ...
        if settings.aux == 0:
            for i in range(settings.action_size):
                qd[i] = trajectory_time.bezier(params_time.a_rightS[:,i],tau_right)    #Compute the desired position for the actuated joints using the current measured state, the control parameters and bezier polynomials
                qdotd[i] = trajectory_time.dbezier(params_time.a_rightS[:,i],tau_right)*self.dtau  #Compute the desired velocity for the actuated joints using the current measured state, the control parameters and bezier polynomials
            reward_tau = tau_right

        else:
            for i in range(settings.action_size):
                qd[i] = trajectory_time.bezier(params_time.a_leftS[:,i],tau_left)    #Compute the desired position for the actuated joints using the current measured state, the control parameters and bezier polynomials
                qdotd[i] = trajectory_time.dbezier(params_time.a_leftS[:,i],tau_right)*self.dtau  #Compute the desired velocity for the actuated joints using the current measured state, the control parameters and bezier polynomials
            reward_tau = tau_left
...
# a_rightS = params_time.a_rightS
# a_leftS = params_time.a_leftS
# p = params_time.p
# Kp, Kd = pid_init() #Initialize PID parameters for njoints = 4 (4 actuators)

# for i in range(1000):
#     env.unwrapped.set_state(params_time.position[0,:],params_time.velocity[0,:])
#    # env.render()
aux = 0
bnd_ctrl_act = 4
plot_mode = False
...
max_episode_length = 1600
iter = 1
settings.sensor1_flag = False
settings.sensor2_flag = False

for k in range(iter):

    velocity_list = []
    sensor1_list = np.zeros(10)
    sensor2_list = np.zeros(10)
    ...
    for i in range(max_episode_length*2):
        ...
        observation, reward_params, done, _info = env.step(action)
        ...
        sensor1_list = np.roll(sensor1_list,1)
        sensor2_list = np.roll(sensor2_list,1)
        sensor1_list[0] = env.get_sensor_data("s_t1")
        sensor2_list[0] = env.get_sensor_data("s_t2")
        average_sensor_1 = np.mean(sensor1_list)
        average_sensor_2 = np.mean(sensor2_list)
        print("av_sensor 1 list= {}" .format(average_sensor_1))
        print("av_sensor 2 list= {}" .format(average_sensor_2))
