...
class Settings():
	...
	theta_dim = 30
	nn_units=[16,16]
	nn_activations=["relu", "relu", "passthru"]  #tanh or passthru
	population = 24
	sigma_init=0.1
	sigma_decay=0.999 
	sigma_limit=1e-4
	...
	upplim_foot = 120*(np.pi/180)
	lowlim_foot = 0*(np.pi/180)
	plot_mode = False
	sigmoid_slope = 1
	init_vel = 0.3769
	def __init__(self):
...
def get_reward(reward_params, reward_tau, desired_vel, mode="linear1"):
	...
	if mode == "quadratic":
		...
		action_reward = -1e-2 * np.sum(a**2)
		displacement_reward  = posafter
		reward = alive_bonus + 20*velocity_reward + 1*action_reward + 1*displacement_reward
		reward = scale*reward	
	...
	if mode == "angular":
		...
		displacement_reward  = posafter
		angular_reward = w
		reward = alive_bonus + 10*velocity_reward + 1*action_reward + 1*displacement_reward + 5*angular_reward
		reward = scale*reward		
...
class Policy():
	...
	def make_theta(self):
		...
		self.a_rightS = np.append(self.theta, [self.theta[3], self.theta[5], self.theta[5], self.theta[0], self.theta[1], self.theta[2]])
...
def simulate(model, solution, settings, desired_velocity, render_mode):
	...
	for episode in range(settings.num_episode):	
		...
		for t in range(settings.max_episode_length):
			...
			reward = get_reward(reward_params, reward_tau, desired_velocity, mode="angular")
			state = observation
...
if __name__ == "__main__":
	...
	while total_timesteps < settings.total_threshold:
		...
		desired_velocities = np.round(desired_velocities)
		desired_velocities = desired_velocities/10
